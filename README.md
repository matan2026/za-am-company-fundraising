...
ערכי העמודים המשפטיים נמצאים ב־`src/config/legal.ts`. יש להגדיר את `privacyLastUpdated` רק לאחר בדיקה ואישור, בפורמט `YYYY-MM-DD`, ולהוסיף `paymentProviderPrivacyUrl` רק אם קיים קישור HTTPS מאומת של ספק הסליקה.
